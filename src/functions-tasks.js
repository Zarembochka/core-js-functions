...
function getPolynom(...args) {
  ...
  function func(x) {
    const bodyFunction = args
      .map((elem, index) => elem * x ** (args.length - 1 - index))
      .reduce((a, b) => a + b);
    return bodyFunction;
  }
  return func;
...
function logger(func, logFunc) {
  return function fn(...args) {
    const parametres = args.map((item) =>
      Array.isArray(item) ? JSON.stringify(item) : item
    );
    logFunc(`${func.name}(${parametres}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${parametres}) ends`);
    return result;
  };
}
